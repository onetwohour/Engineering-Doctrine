...
컨텍스트 압축 뒤에는 Claude Code가 이미 호출된 스킬을 다시 붙입니다. 스킬당 앞쪽 5,000토큰까지, 합계 25,000토큰 예산 안에서 가장 최근에 호출한 것부터 채웁니다. doctrine 스킬은 각각 스킬당 한도 안에 들어가지만 12개를 합치면 약 26,800토큰이라, 대부분을 로드한 세션에서는 먼저 호출한 것이 빠질 수 있습니다. 그때는 다시 호출하면 되고, 이름을 직접 불러 강제할 수도 있습니다. `/engineering-doctrine:mutation-safety`처럼 쓰면 됩니다.
