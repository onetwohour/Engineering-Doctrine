...
// A batch of only these changes no doctrine state worth reporting.
const OBSERVING_TOOLS = new Set(['Read', 'Grep', 'Glob', 'WebFetch', 'WebSearch', 'ToolSearch', 'Skill', 'ListAgents', 'TaskOutput']);
const SHELL_TOOLS = new Set(['Bash', 'PowerShell']);
// Re-issue an unchanged report this often so it cannot drift out of reach.
...
// Keep every answer below derived from the transcript: storing state here would make the runtime a
// second classifier.
...
// Both fingerprints are required: the summary stops matching a body an older build delivered once
// the doctrine edits that summary, and the preamble is the harness's wording to change.
...
// No delivery at all despite invocations means the fingerprints stopped matching, not that nothing
// was delivered; the caller falls back rather than blocking every edit on a broken signal.
