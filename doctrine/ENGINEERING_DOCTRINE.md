...
<!-- doctrine-applicability
{
  ...
  "stages": {
    ...
    "plan": {
      "when": "sequencing implementation, migration, compatibility, dependencies, or verification, or recording the provenance of a change",
  ...
  "surfaces": {
    ...
    "documentation": {
      "when": "permanent or generated documentation, or an agent-facing instruction file, is meaningfully changed or reviewed",
...
<!-- doctrine-rule {"id":"design.change-provenance","authority":"binding","applies":{"kind":"stage","value":"plan"}} -->
...
<!-- doctrine-rule {"id":"documentation.agent-instructions","authority":"binding","applies":{"kind":"surface","value":"documentation"}} -->
...
Prefer an environment-provided task-state, workspace-memory, or other non-repository continuity mechanism when one exists. Do not create a repository file solely as model memory unless the repository already defines such a convention or the owner explicitly authorizes it. Committed change-provenance artifacts are a different thing: the record of why a change was needed, what was agreed, and how it would be built has its own readers and its own lifetime, and belongs to the project rather than to this session. Do not hide task state inside product documentation, source comments, configuration, tests, or other durable artifacts that have a different owner.
