...
function compileDoctrine(src,runtimeSource){
  ...
  const hookConfig={description:"Loads the Engineering Doctrine governing kernel and foregrounds §0.1 semantic reassessment at native decision points. Hooks add context only; they never classify, invalidate, or block.",hooks:{SessionStart:[{matcher:"startup|resume|clear|compact|fork",hooks:[runtimeHook("session-context")]}],SubagentStart:[{matcher:".*",hooks:[runtimeHook("subagent-context")]}],UserPromptSubmit:[{hooks:[runtimeHook("prompt")]}],PostToolUseFailure:[{matcher:".*",hooks:[runtimeHook("failure")]}],PostToolBatch:[{hooks:[runtimeHook("batch")]}]}};
  const hookModes=new Set(["session-context","subagent-context","prompt","failure","batch"]);
