...
function compileDoctrine(src,runtimeSource){
  ...
  for(const skill of skills.values()){
    ...
    const labels=skill.routes.map(route=>{const value=cue(parsed.tax,route.signal);if(!value)throw new Error("missing discovery cue "+route.signal);return value.replace(/[.;]+$/,"");});skill.description="Invoke this Doctrine skill when current work involves "+joinAlternatives(labels)+".";
